...
const OWNED_ROOT_KEYS = new Set(["command", "url", "args", "env_vars", "bearer_token_env_var"]);
const OWNED_SUBSECTIONS = new Set(["env", "http_headers", "env_http_headers"]);

export function installCodex(skills: Skill[], mcps: McpConfig[], global: boolean, cwd: string): void {
  if (global) {
...
function mergeMcpsToml(mcps: McpConfig[], configPath: string, displayPath: string): void {
...
function buildStdioTomlSection(name: string, config: StdioMcpTransportConfig): string {
...
function buildHttpTomlSection(name: string, config: HttpMcpTransportConfig): string {
  ...
  if (headers && typeof headers === "object" && !Array.isArray(headers)) {
    for (const [key, value] of Object.entries(headers)) {
      if (typeof value !== "string") {
        throw new Error(`Unsupported MCP config for ${name}: headers.${key} must be a string for Codex`);
      }
...
function extractTomlSections(
  ...
  const sections = parsedSections.filter(
    (section) => section.name === sectionPrefix || section.name.startsWith(sectionPrefix + "."),
  );
...
function mergeTomlSection(existing: string, emitted: string, sectionPrefix: string): string {
...
function mergeTomlSections(existingSections: TomlSection[], emittedSections: TomlSection[]): TomlSection[] {
  if (emittedSections.length === 0) {
...
function assertNoUnsupportedPlaceholder(mcpName: string, field: string, value: string): void {
  if (!containsEnvPlaceholderSyntax(value)) {
    return;
  }
  if (extractEnvVar(value) || extractBearerTokenEnvVar(value)) {
    return;
  }
