...
- `src/structured.ts` — shared JSON/TOML parse/stringify + `structuredKind(relPath)` (the one place that decides a file is JSON, TOML, or opaque bytes). Used by the overlay merge, `owned-keys.ts`, and capture.
- `src/machine-owned.ts` — loads `config/machine-owned.json` (target → file → owned top-level key names) once per config phase into `ownedKeysFor(target, relPath)`; absence returns `[]` everywhere, keeping the feature fully inert with no manifest. Warns and skips a bad entry (unknown target, non-structured file, non-string-array); throws on an unreadable/malformed manifest so a phase never proceeds without splice protection.
- `src/owned-keys.ts` — the two primitives declared keys drive: `stripOwnedKeys` (parse, delete the owned keys, canonical key-sorted re-stringify — used for hashing, so owned-key churn and formatting differences don't count as drift) and `spliceOwnedKeys` (repo content with the destination's state — value or absence — grafted on for each owned key — used for writing, so a sync never resets one). Inverse of the target-side `mergeTargetConfig` MCP-merge helper, which is unrelated and unchanged.
- `src/config-install.ts` — target-agnostic install/drift phase. Expands `${VAR}` (files with unset vars are skipped and reported), then writes with drift-aware overwrite: a destination is overwritten only when its content matches the hash AI Kit last recorded; drifted or never-managed destinations are skipped unless `--force`. `configPhase` runs inside a global `install` before the MCP merge; `finalizeMcpManagedHashes` re-hashes shared files (codex `config.toml`, opencode `opencode.json`) after the MCP merge so the recorded hash covers the final content and the next sync sees no self-drift. A file with machine-owned keys (`resolveOwnedFile`) is drift-checked and hashed on its stripped form (a legacy raw hash still passes once, migrating state with no manual step) and written via `spliceOwnedKeys` — even under `--force`; `rehashMcpManaged` strips owned keys too when re-hashing after the MCP merge.
- `src/config-capture.ts` — the reverse direction: copies tracked ∪ curated files from a config root into the base tree, strips AI Kit-rendered MCP sections, and warns on `${VAR}` replacement and overlay-contributed values. For a file with machine-owned keys, `restoreOwnedKeys` swaps `spliceOwnedKeys`'s sides so the captured copy keeps the repo's own value (or absence) for each owned key — a machine's value for one never crosses into the repo; an unparseable side skips the file entirely rather than capturing it raw.
